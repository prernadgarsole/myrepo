...
Add-Content $UCD_Report -Value "Application Name,Environment Name,User Name,Start Time,End Time,Snapshot Name,Snapshot Version,Component Name,Component Version,Used Snapshot"
...
$reportJson.items | foreach {
    ...
    $items | foreach {
        ...
        if ($status -eq "SUCCESS")
        {
            ...
            $line = "$AppName,$EnvName,$userName,$startTime,$endTime,$SnapshotName,$SnapshotID"

            # Check if the environment is non-production (UNIT, INTG, etc.)
            ...
            if ($nonProdEnvs -contains $EnvName)
            {
                ...
                # Extract component details from the manifest (components are returned in the 'components' field)
                ...
                # Add 'No' for Used Snapshot as we're using the deployment manifest API
                $line = $line + ",No"
            }
            else
            {
                ...
                # Initialize the indicator for snapshot usage
                $usedSnapshot = "No"
                
                # Extract component details from the snapshot response
                ...
                # Check if snapshot data is present and update the Used Snapshot column
                if ($snapshotJson -ne $null -and $snapshotJson.Count -gt 0)
                {
                    $usedSnapshot = "Yes"
                }

                # Add snapshot indicator
                $line = $line + "," + $usedSnapshot
            }
